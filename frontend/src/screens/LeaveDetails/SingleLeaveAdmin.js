...
function SingleLeaveAdmin(props) {
    ...
    const approveLeave = async() => {
        ...
        for(var i=0;i<response.data.length;i++){
            if(response.data[i].Status == "Approved by HOD" && response.data[i].LeaveID == props.LeaveID){
                console.log("called");
...
export default SingleLeaveAdmin